...
#Best set of hyperparameters for Q3

def parse_arguments(parser):
    parser.add_argument("--lr", type=float, default=0.0006161)
    parser.add_argument("--epoch",type=int, default=1)
    parser.add_argument("--hidden_layer_dim",type=int, default=256)
    parser.add_argument("--iteration_conv",type=int, default=10)
    parser.add_argument("--steps_chain",type=int, default=200)
...
#Evaluate the learnt represetnation
...
#Utility function to convert the dataset into binary format for training RBMs  
def binary(data):
  for i in range(len(data)):
...
#Train - 2 modes: 1- Contrastive Divergence 2 - Blocked Gibbs
...
#__contrastiveDivergence compute step
  ...
  def __contrastiveDivergence(self, data):
    ...
    for i in range(self.epochs):
      ...
      for j in tqdm(data):
        ...
        self.__CD_compute(j)
        samples += 1
        if samples % int(len(data)/64) == 0:
          count +=1
          self.vs.append(self.v_p.reshape(28,28))
          labels.append("Sample {}".format(count))
          # wandb.log({"Sample {}".format(count):wandb.Image(self.v_p.reshape(28,28), caption="Sample {}".format(count))})
          if count == 64:
            break
      wandb.log({"Samples Learned After Convergence":[wandb.Image(img, caption=label)for img, label in zip(self.vs,labels)]})
